...
if (mysqli_num_rows(mysqli_query($dbc, "SELECT url FROM site")) != 0) {
	...
	$countsites = mysqli_fetch_row(mysqli_query($dbc, "SELECT COUNT(pile.site_id) FROM site INNER JOIN category USING(cat_id) LEFT JOIN pile ON pile.site_id=site.site_id $queryparameter"));
	...
	$listsites = mysqli_query($dbc, "SELECT site.cat_id, site.site_id, site.url, site.title, category.cat_name FROM site INNER JOIN category USING(cat_id) $queryparameter ORDER BY " . mysqli_real_escape_string($dbc, trim($orderby)) . " $ordersort LIMIT $start, $display");

	$listmysites = mysqli_query($dbc, "SELECT * FROM site INNER JOIN category USING(cat_id) INNER JOIN pile USING(site_id) $queryparameter ORDER BY " . mysqli_real_escape_string($dbc, trim($orderby)) . " $ordersort LIMIT $start, $display");
	...
	if (!$listmysites) {
		echo 'Query error.';
	} elseif (mysqli_num_rows($listmysites) == 0) {
		...
		while ($row = mysqli_fetch_assoc($listmysites)) {
			...
			echo '<tr>
			<td><a href="?action=add&site_id=' . $row['site_id'] . '&' . $urlgetarrayvalues . '">';
			...
			echo '<td><a href="' . $row['url'] . '" target="_blank">' . htmlentities(capstring($row['url'], 25)) . '</a></td>';
			echo '</tr>';
		}
		echo '</table><br/>';

		if (mysqli_num_rows($listmysites) != 1) {
